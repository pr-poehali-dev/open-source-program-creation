...
// Встроенная база знаний ассистента ЕГСУ
const KB: { pattern: RegExp; answer: string }[] = [
  {
    pattern: /привет|здравствуй|добрый/i,
    answer: "Добро пожаловать в систему ЕГСУ 2.0! Я — ваш ИИ-ассистент координатора. Могу помочь с анализом инцидентов, рекомендациями по реагированию и информацией о системе. Что вас интересует?",
  },
  {
  ...
  },
  {
    ...
    answer: "Текущий uptime системы ЕГСУ 2.0: 99.9%. Задержка API: менее 48 мс. За сегодня обработано 10.2 миллиона запросов. Все подсистемы работают в штатном режиме.",
  },
  {
    pattern: /орган|совет|суд|группа реагировани/i,
    answer: "Активные органы ЕГСУ: Глобальный совет безопасности (45 участников), Международный суд справедливости (15 судей), Оперативная группа расследования (120 агентов), Силы быстрого реагирования (5000 специалистов), Межпарламентский совет (94 представителя), Комиссия по этике и науке (24 эксперта).",
  },
  {
    pattern: /ии|искусственный интеллект|модел|нейросет/i,
    answer: "В ЕГСУ 2.0 работают 3 ИИ-модели: 1) Классификатор инцидентов (CNN v2.1) — точность 78%, 2) Детектор аномалий (LSTM v1.8) — точность 84%, 3) Генератор отчётов NLP (Transformer v3) — точность 91%. Все модели проходят регулярный аудит прозрачности.",
  },
  {
    pattern: /закон|право|конвенц|легальн/i,
    answer: "Все действия ЕГСУ основаны на: Парижском соглашении, Конвенции о биологическом разнообразии, Орхусской конвенции, Будапештской конвенции по киберпреступности, Директиве NIS2 (ЕС), протоколах ЮНЕП и МАРПОЛ. Принудительные меры применяются только с санкции уполномоченных органов.",
  },
  {
  ...
  },
  {
    ...
    answer: "Статистика ЕГСУ за апрель 2026: Экология — 45 инцидентов (45%), Вода — 28 (28%), Воздух — 19 (19%), Кибер — 8 (8%). Пиковый день недели: четверг (24 инцидента). Средняя скорость реагирования — 2.3 часа. Процент успешного закрытия — 71.6%.",
  },
];

const DEFAULT_ANSWER = "Понял ваш вопрос. По данной теме у меня недостаточно информации в текущей базе знаний ЕГСУ. Попробуйте уточнить: спросите о конкретном инциденте (например, 'расскажи про Нигерию'), статистике, органах системы или правовой базе.";
...
export default function AiChat({ onClose }: Props) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      text: "Система ЕГСУ 2.0 онлайн. Я — ИИ-ассистент координатора. Готов помочь с анализом инцидентов и рекомендациями по реагированию.",
      time: getTime(),
    },
  ]);
    ...
    <div className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl overflow-hidden flex flex-col"
      ...
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-white text-sm font-bold leading-none">ИИ-ассистент ЕГСУ</div>
